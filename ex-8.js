...
// Start coding here
let obj = {};
let mapItem = bills.map((item) => item.location);
let uniqueLocation = mapItem.filter(
  (item, index) => mapItem.indexOf(item) === index
);

function totalReduce(location) {
  let sum = 0;
  for (let i = 0; i < bills.length; i++) {
    if (bills[i].location === location) {
      sum += bills[i].total;
    }
  }
...
for (let i = 0; i < bills.length; i++) {
  obj[uniqueLocation[i]] = totalReduce(uniqueLocation[i]);
}
